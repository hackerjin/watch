...
namespace Skyray.Print
{
    public class ClassHelper
    {
        ...
        public static Type BuildType(string className)
        {
            ...
            SaveAssemblyBuilder(myAsmName, myAsmBuilder, AppDomain.CurrentDomain.BaseDirectory);
        ...
        /// 保存程序集的路径
        /// </summary>
        /// <param name="myAsmName"></param>
        /// <param name="myAsmBuilder"></param>
        private static void SaveAssemblyBuilder(AssemblyName myAsmName, AssemblyBuilder myAsmBuilder,string path)
        {
            var type = myAsmBuilder.GetType();
            var property = type.GetProperty("m_assemblyData", BindingFlags.NonPublic | BindingFlags.Instance);
            var propertyType = property.PropertyType;
            var field = propertyType.GetField("m_strDir", BindingFlags.NonPublic | BindingFlags.Instance);
            field.SetValue(property.GetValue(myAsmBuilder, null), path);
            myAsmBuilder.Save(myAsmName.Name + ".dll");
        }
