...
{    
    ...
    /// </summary>
    public class TrackBarW : TrackBar
    {
        #region Fields
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            else
            {
                ...
                if (m.Msg == 0x204e)
                {
                    TNativeMethods.NMHDR structure = (TNativeMethods.NMHDR)Marshal.PtrToStructure(m.LParam, typeof(TNativeMethods.NMHDR));
                    if (structure.code == -12)
                    {
                        IntPtr ptr;
                        Marshal.StructureToPtr(structure, m.LParam, false);
                        TNativeMethods.NMCUSTOMDRAW nmcustomdraw = (TNativeMethods.NMCUSTOMDRAW)Marshal.PtrToStructure(m.LParam, typeof(TNativeMethods.NMCUSTOMDRAW));
                        if (nmcustomdraw.dwDrawStage == TNativeMethods.CustomDrawDrawStage.CDDS_PREPAINT)
                        {
                        ...
                        else if (nmcustomdraw.dwDrawStage == TNativeMethods.CustomDrawDrawStage.CDDS_POSTPAINT)
                        {
                            this.OnDrawTicks(nmcustomdraw.hdc);
                            this.OnDrawChannel(nmcustomdraw.hdc);
                            this.OnDrawThumb(nmcustomdraw.hdc);
                        }
                        else if (nmcustomdraw.dwDrawStage == TNativeMethods.CustomDrawDrawStage.CDDS_ITEMPREPAINT)
                        {
                            if (nmcustomdraw.dwItemSpec.ToInt32() == 2)
                            {
                                ...
                                if (this.Enabled)
                                {
                                    if (nmcustomdraw.uItemState == TNativeMethods.CustomDrawItemState.CDIS_SELECTED)
                                    {
                                        this.ThumbState = 3;
                                    }
                                    else
        ...
        protected virtual void OnDrawTicks(IntPtr hdc)
        {
            ...
            else
            {
                ...
                {
                    VisualStyleRenderer vsr = new VisualStyleRenderer("TRACKBAR", (int)TNativeMethods.TrackBarParts.TKP_TICS, ThumbState);
        ...
        protected virtual void OnDrawThumb(IntPtr hdc)
        {
            ...
            else
            {
                ...
                TNativeMethods.TrackBarParts part = TNativeMethods.TrackBarParts.TKP_THUMB;
                if (this.ThumbBounds.Equals(Rectangle.Empty))
                {
                    return;
                }
                switch (this.TickStyle)
                {
                    ...
                    case TickStyle.BottomRight:
                        part = (Orientation != Orientation.Horizontal) ? TNativeMethods.TrackBarParts.TKP_THUMBRIGHT : TNativeMethods.TrackBarParts.TKP_THUMBBOTTOM;
                        break;
                    case TickStyle.TopLeft:
                        part = (this.Orientation != Orientation.Horizontal) ? TNativeMethods.TrackBarParts.TKP_THUMBLEFT : TNativeMethods.TrackBarParts.TKP_THUMBTOP;
                        break;

                    case TickStyle.Both:
                        part = (this.Orientation != Orientation.Horizontal) ? TNativeMethods.TrackBarParts.TKP_THUMBVERT : TNativeMethods.TrackBarParts.TKP_THUMB;
                        break;
                }
                // Perform drawing
                if (VisualStyleRenderer.IsSupported)
                ...
                else
                {
                    switch (part)
                    {
                        case TNativeMethods.TrackBarParts.TKP_THUMBBOTTOM:
                            this.DrawPointerDown(graphics);
                            break;
                        case TNativeMethods.TrackBarParts.TKP_THUMBTOP:
                            this.DrawPointerUp(graphics);

                            break;
                        case TNativeMethods.TrackBarParts.TKP_THUMBLEFT:
                            this.DrawPointerLeft(graphics);

                            break;
                        case TNativeMethods.TrackBarParts.TKP_THUMBRIGHT:
        ...
        protected virtual void OnDrawChannel(IntPtr hdc)
        {
            ...
            else
            {
                ...
                if (VisualStyleRenderer.IsSupported)
                {
                    VisualStyleRenderer vsr = new VisualStyleRenderer("TRACKBAR", (int)TNativeMethods.TrackBarParts.TKP_TRACK, (int)TrackBarItemState.Normal);
