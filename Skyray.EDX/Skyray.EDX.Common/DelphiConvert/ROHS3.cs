...
namespace Skyray.EDX.Common.DelphiConvert
{
    public class ROHS3 : IDevice
    {
        ...
        public void ReadFileCollimator(string filePath, Device device)
        {
            ...
            int size = 255;
            TxtFile.GetPrivateProfileString("CollMotor", "Exist", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            if (Convert.ToInt32(temp.ToString()) == 1)
                device.HasCollimator = true;
            TxtFile.GetPrivateProfileString("CollMotor", "MotorID", "1", temp, size, filePath + "\\Configure\\Parameter.ini");
            device.CollimatorElectricalCode = Convert.ToInt32(temp.ToString());
            TxtFile.GetPrivateProfileString("CollMotor", "MotorDir", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            device.CollimatorElectricalDirect = Convert.ToInt32(temp.ToString());
            TxtFile.GetPrivateProfileString("CollMotor", "Speed", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            device.CollimatorSpeed = Convert.ToInt32(temp.ToString());
            int colliCount = 0;
            for (int i = 0; i < 8; i++)
            {
                TxtFile.GetPrivateProfileString("CollMotor", "Tag" + (i + 1), "0", temp, size, filePath + "\\Configure\\Parameter.ini");
        ...
        public void ReadFileFilter(string filePath, Device device)
        {
              StringBuilder temp = new StringBuilder(255);
            int size = 255;
            TxtFile.GetPrivateProfileString("FilterMotor", "Exist", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            if (Convert.ToInt32(temp.ToString()) == 1)
                device.HasFilter = true;
            TxtFile.GetPrivateProfileString("FilterMotor", "MotorID", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            device.FilterElectricalCode = Convert.ToInt32(temp.ToString());
            TxtFile.GetPrivateProfileString("FilterMotor", "MotorDir", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            device.FilterElectricalDirect = Convert.ToInt32(temp.ToString());
            TxtFile.GetPrivateProfileString("FilterMotor", "Speed", "0", temp, size, filePath + "\\Configure\\Parameter.ini");
            device.FilterSpeed = Convert.ToInt32(temp.ToString());
            int filterCount = 0;
            for (int i = 0; i < 8; i++)
            {
                TxtFile.GetPrivateProfileString("FilterMotor", "Tag" + (i+1), "0", temp, size, filePath + "\\Configure\\Parameter.ini");
                if (Convert.ToInt32(temp.ToString()) == 0)
                    continue;
        ...
        public void IsExistMoveAis(string filePath, Device device)
        {
            ...
            filePath += "\\Configure\\Parameter.ini";
            StringBuilder temp = new StringBuilder(255);
            int size = 255;
